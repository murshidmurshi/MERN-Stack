...
export default function ShoppingCart(props) {
  ...
  function calculateTotal(items) {
    let total = 0;
    items.forEach((item) => {
      const price = item.grocery_id.price;
      ...
      {cart.map((item, index) => {
        return (
          <div key={index} className="box">
            ...
            <img src={item.grocery_id.image} height={100} width={100} alt="Product" />
            <div className="content">
              <h3>{item.grocery_id.name}</h3>
              <span className="price">Rs {item.grocery_id.price}</span>
